...
document.addEventListener("DOMContentLoaded", function () {
    const dbRef = firebase.database().ref('educationLevels');
    const tableBody = document.getElementById("educationLevels-table-body");
    const form = document.getElementById("educationLevels-form");
        ...
        dbRef.once("value", (snapshot) => {
            ...
            snapshot.forEach((childSnapshot) => {
                ...
                const row = `<tr>` + Object.values(data).map(v => `<td>${v}</td>`).join("") + 
                            `<td>
                                <button onclick="editItem('educationLevels', '${key}')">✏️</button>
                                <button onclick="deleteItem('educationLevels', '${key}')">🗑️</button>
                             </td></tr>`;
