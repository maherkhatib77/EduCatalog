...
document.addEventListener("DOMContentLoaded", function () {
    const dbRef = firebase.database().ref('generalInfo');
    const tableBody = document.getElementById("generalInfo-table-body");
    const form = document.getElementById("generalInfo-form");
        ...
        dbRef.once("value", (snapshot) => {
            ...
            snapshot.forEach((childSnapshot) => {
                ...
                const row = `<tr>` + Object.values(data).map(v => `<td>${v}</td>`).join("") + 
                            `<td>
                                <button onclick="editItem('generalInfo', '${key}')">✏️</button>
                                <button onclick="deleteItem('generalInfo', '${key}')">🗑️</button>
                             </td></tr>`;
