...
document.addEventListener("DOMContentLoaded", function () {
    const dbRef = firebase.database().ref('learningCategories');
    const tableBody = document.getElementById("learningCategories-table-body");
    const form = document.getElementById("learningCategories-form");
        ...
        dbRef.once("value", (snapshot) => {
            ...
            snapshot.forEach((childSnapshot) => {
                ...
                const row = `<tr>` + Object.values(data).map(v => `<td>${v}</td>`).join("") + 
                            `<td>
                                <button onclick="editItem('learningCategories', '${key}')">✏️</button>
                                <button onclick="deleteItem('learningCategories', '${key}')">🗑️</button>
                             </td></tr>`;
