...
document.addEventListener("DOMContentLoaded", function () {
    const dbRef = firebase.database().ref('learningModes');
    const tableBody = document.getElementById("learningModes-table-body");
    const form = document.getElementById("learningModes-form");
        ...
        dbRef.once("value", (snapshot) => {
            ...
            snapshot.forEach((childSnapshot) => {
                ...
                const row = `<tr>` + Object.values(data).map(v => `<td>${v}</td>`).join("") + 
                            `<td>
                                <button onclick="editItem('learningModes', '${key}')">✏️</button>
                                <button onclick="deleteItem('learningModes', '${key}')">🗑️</button>
                             </td></tr>`;
