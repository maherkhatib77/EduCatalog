...
document.addEventListener("DOMContentLoaded", function () {
    const dbRef = firebase.database().ref('meetingDays');
    const tableBody = document.getElementById("meetingDays-table-body");
    const form = document.getElementById("meetingDays-form");
        ...
        dbRef.once("value", (snapshot) => {
            ...
            snapshot.forEach((childSnapshot) => {
                ...
                const row = `<tr>` + Object.values(data).map(v => `<td>${v}</td>`).join("") + 
                            `<td>
                                <button onclick="editItem('meetingDays', '${key}')">✏️</button>
                                <button onclick="deleteItem('meetingDays', '${key}')">🗑️</button>
                             </td></tr>`;
