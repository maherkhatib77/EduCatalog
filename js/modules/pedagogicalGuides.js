...
document.addEventListener("DOMContentLoaded", function () {
    const dbRef = firebase.database().ref('pedagogicalGuides');
    const tableBody = document.getElementById("pedagogicalGuides-table-body");
    const form = document.getElementById("pedagogicalGuides-form");
        ...
        dbRef.once("value", (snapshot) => {
            ...
            snapshot.forEach((childSnapshot) => {
                ...
                const row = `<tr>` + Object.values(data).map(v => `<td>${v}</td>`).join("") + 
                            `<td>
                                <button onclick="editItem('pedagogicalGuides', '${key}')">✏️</button>
                                <button onclick="deleteItem('pedagogicalGuides', '${key}')">🗑️</button>
                             </td></tr>`;
